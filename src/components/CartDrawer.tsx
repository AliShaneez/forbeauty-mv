  ...
  return (
    <div className={`fixed inset-y-0 right-0 w-full max-w-md bg-white shadow-xl transition-transform ${isOpen ? 'translate-x-0' : 'translate-x-full'} z-40`}>
      <div className="flex h-16 items-center justify-between border-b border-primary px-4">
        ...
        <button className="text-charcoal hover:text-secondary" onClick={() => toggle(false)}>Close</button>
      </div>
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {items.length === 0 && <div className="text-sm text-charcoal">Your cart is empty.</div>}
        {items.map(i => (
          <div key={`${i.product.id}-${i.shade ?? ''}`} className="flex gap-3 border border-primary rounded-lg p-3">
            <img src={i.product.imageUrl} alt={i.product.name} className="h-16 w-16 object-cover rounded" />
            <div className="flex-1">
              <div className="text-sm text-charcoal">{i.product.brand}</div>
              <div className="font-medium">{i.product.name}</div>
              {i.shade && <div className="text-xs text-charcoal">Shade: {i.shade}</div>}
              <div className="mt-1 text-charcoal">{formatCurrency(i.product.price)}</div>
